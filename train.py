...
if __name__ == '__main__':
    ...
    # Model parameters
    parser.add_argument('--input_dim',type=int, default=2, metavar='IN',  # --input_dim as the examples above the -- dashes
                        help='input_dim for features to model (default: 2)') # 2 dimensional data
    parser.add_argument('--hidden_dim',type=int, default=10, metavar='H',
                        help='hidden_dim of model (default: 10)') 
    parser.add_argument('--output_dim',type=int, default=1, metavar='OUT',
                        help='output_dim for training (default: 1)') # I just want to ouput one class core
    ...
    model = SimpleNet(args.input_dim, args.hidden_dim, args.output_dim).to(device) # Instantiate the model
    ...
    optimizer = optim.Adam(model.parameters(),lr=args.lr) # You can use stochastic gradient descent instead of Adam

    criterion = nn.BCELoss() # BCELoss() returns one value. Cannot use torch.nn.CrossEntropyLoss() because returns several values
